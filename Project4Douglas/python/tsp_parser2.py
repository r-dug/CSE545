...
def scg_costs(nodes, connections):
    node_names = list(nodes.keys())
    for i in node_names:
        current = nodes[i]
        x1, y1 = current["xy"][0], current["xy"][1]
...
class Parser:   
    def __init__(self, 
                 path, 
                 num_nodes=None, 
                 graph_type = "fc", # fc = fully connected, sc = strongly connected
                 verbose = False):
        ...
        if self.graph_type == "sc":
            self.connections = random_edges(self.nodes)
        if self.graph_type == "fc":
            self.nodes = fcg_costs(self.nodes)
        elif self.graph_type == "sc":
            self.nodes = scg_costs(self.nodes, self.connections)
        self.tour_costs = []
        self.runtimes = {}
